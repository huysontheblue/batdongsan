  ...
  <section class="hero" style="background-image: url('{{ asset('frontend/assets/images/bg.jpg') }}')">
    <div class="container">
      <div class="row text-center" style="padding-top: 270px">
        <h3 class="text-white">
          <span class="fw-bold text-dark">Tìm</span> ngôi nhà <br />
            mơ ước <span class="fw-bold text-dark">của bạn</span>
            </h3>
          </div>
        </div>
      </section>
          ...
          <div class="card col-lg-8 p-4 border-0 shadow">
            <div class="row text-center">
              <div class="col">
                ...
                <span class="text-secondary">Bất động sản đã bán</span>
              </div>
              <div class="col">
                <span class="fw-bold fs-1">123+</span> <br />
                <span class="text-secondary">Khách hàng hạnh phúc</span>
              </div>
              <div class="col">
                <span class="fw-bold fs-1">100+</span> <br />
                <span class="text-secondary">Nhà đầu tư</span>
              </div>
              <div class="col">
      ...
      @foreach($categories as $category)
        <section class="container category" style="margin-bottom: 100px">
          <div class="d-flex justify-content-between align-items-center">
            <h3>{{ $category->name }}</h3>
            <a href="{{ route('category.index', $category->slug) }}" style="height: 50px" class="d-flex px-5 align-items-center btn btn-outline-dark">Xem thêm</a>
          </div>
          <hr />
            ...
            @foreach($category->properties as $property)
              <div class="col-lg-4 mb-4">
                <div class="card border-0 shadow-sm">
                  <img height="310" style="object-fit: cover" src="{{ Storage::url($property->galleries()->first()->path) }}" class="card-img-top"/>
                  <div class="card-body">
                    <h5 class="card-title">
                      {{ $property->name }}
                    </h5>
                    ...
                    <a href="{{ route('property.show', $property->slug) }}" style="right: 0; bottom: 0; border-radius: 0.4rem 0 0.4rem 0"class="align-items-center border-0 p-3 px-5 position-absolute btn btn-primary d-inline-flex">xem chi tiết</a>
                  </div>
                </div>
              </div>
            @endforeach
